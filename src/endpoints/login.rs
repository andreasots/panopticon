...
pub fn form(mut state: State) -> Box<HandlerFuture> {
    ...
    let response = try_h!(state, state.borrow::<Templates>().render("login.html", &context));
...
pub fn login(mut state: State) -> Box<HandlerFuture> {
    ...
    let (user, pass) = {
        let (user, pass) = {
            ...
            (req.get_first("username").map(String::from), req.get_first("password").map(String::from))
        };
        match (user, pass) {
            (Some(user), Some(pass)) => (user, pass),
            (user, pass) => {
                state.borrow_mut::<SessionData<Session>>().flash("Incorrect username or password.");
    ...
    let user = match try_h!(state, users.filter(name.eq(&user)).first::<User>(&conn).optional()) {
        Some(user) => user,
        None => {
            state.borrow_mut::<SessionData<Session>>().flash("Incorrect username or password.");
            let mut context = Context::new(&mut state);
    ...
    state.borrow_mut::<SessionData<Session>>().flash("Logged in successfully.");
